...
description: |
  ...
  configured, this skill calls `mcp_dormy_memory_recall(tags=['gtm', 'strategy'])`
...
> Adapted from upstream `marketingskills/pricing-strategy` (MIT). Source preserved
> verbatim under `dormy-skills/sources/marketingskills/skills/pricing-strategy/`.
...
1. `mcp_dormy_memory_recall(query="<user's specific prompt>", tags=['gtm', 'strategy'])`

...
description: |
  ...
  configured, this skill calls `mcp_dormy_memory_recall(tags=['gtm', 'cro'])`
...
> Adapted from upstream `marketingskills/paywall-upgrade-cro` (MIT). Source preserved
> verbatim under `dormy-skills/sources/marketingskills/skills/paywall-upgrade-cro/`.
...
1. `mcp_dormy_memory_recall(query="<user's specific prompt>", tags=['gtm', 'cro'])`

...
description: |
  ...
  configured, this skill calls `mcp_dormy_memory_recall(tags=['gtm', 'foundations'])`
...
> Adapted from upstream `marketingskills/marketing-psychology` (MIT). Source preserved
> verbatim under `dormy-skills/sources/marketingskills/skills/marketing-psychology/`.
...
1. `mcp_dormy_memory_recall(query="<user's specific prompt>", tags=['gtm', 'foundations'])`

...
description: |
  ...
  configured, this skill calls `mcp_dormy_memory_recall(tags=['gtm', 'copy'])`
...
> Adapted from upstream `marketingskills/email-sequence` (MIT). Source preserved
> verbatim under `dormy-skills/sources/marketingskills/skills/email-sequence/`.
...
1. `mcp_dormy_memory_recall(query="<user's specific prompt>", tags=['gtm', 'copy'])`
